...
ssh -i "$SSH_KEY" ubuntu@$PROD_SERVER bash << 'ENDSSH'
  cd /shared/plugins
  if [ -d "discourse-vehicle-plugin" ]; then
...
ssh -i "$SSH_KEY" ubuntu@$PROD_SERVER bash << 'ENDSSH'
  sudo docker exec app bash -c "
    if [ ! -L /var/www/discourse/plugins/discourse-vehicle-plugin ]; then
      ln -sf /shared/plugins/discourse-vehicle-plugin /var/www/discourse/plugins/discourse-vehicle-plugin
